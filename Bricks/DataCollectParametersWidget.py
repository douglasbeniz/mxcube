    ...
    def validateDirectory(self,directory=None):
        ...
        if self.hasAcceptableInput(directory):
            if os.path.isdir(directory):
                self.setPaletteBackgroundColor(DataCollectParametersWidget.PARAMETER_STATE["OK"])
            else:
                self.setPaletteBackgroundColor(DataCollectParametersWidget.PARAMETER_STATE["WARNING"])
        else:
            valid=False
...
class CheckBoxInput2(QWidget):
    ...
    def txtChanged(self,txt):
        ...
        if self.hasAcceptableInput():
            valid=True
            self.lineEdit.setPaletteBackgroundColor(DataCollectParametersWidget.PARAMETER_STATE["OK"])
        else:
            valid=False
            self.lineEdit.setPaletteBackgroundColor(DataCollectParametersWidget.PARAMETER_STATE["INVALID"])
        self.emit(PYSIGNAL("textChanged"),(txt,))
...
class DataCollectParametersWidget(QWidget):
    ...
    PARAMETERS = (
        ("directory","Directory", 0, 0, 2, DirectoryInput, ("Browse",), None, None, ("PYSIGNAL","PYSIGNAL")),\
        ...
        ("osc_start","Oscillation start (deg)", 1, 2, 0, LineEditInput, (), QWidget.AlignRight, (QDoubleValidator,), ()),\
        ("osc_range","Oscillation range (deg)", 2, 2, 0, LineEditInput, (), QWidget.AlignRight, (QDoubleValidator,), ()),\
        ("overlap","Overlap (deg)", 3, 2, 0, LineEditInput, (), QWidget.AlignRight, (QDoubleValidator,), ()),\
        ("exposure_time","Exposure time (s)", 4, 2, 0, LineEditInput, (), QWidget.AlignRight, (QDoubleValidator,0.0), ()),\
        ...
        ("mad_1_energy","", 1, 4, 0, CheckBoxInput, (), None, None, (None, None, None, "PYSIGNAL")),\
        ("mad_2_energy","", 2, 4, 0, CheckBoxInput, (), None, None, (None, None, None, "PYSIGNAL")),\
        ("mad_3_energy","", 3, 4, 0, CheckBoxInput, (), None, None, (None, None, None, "PYSIGNAL")),\
        ("mad_4_energy","", 4, 4, 0, CheckBoxInput, (), None, None, (None, None, None, "PYSIGNAL")),\
    ...
    def __init__(self, parent, brick):
        ...
        for param in DataCollectParametersWidget.PARAMETERS:
            ...
            self.paramsBox.layout().addMultiCellWidget(input_widget, param_row, param_row, param_column+1, param_column+1+param_span)
        ...
        self.paramsBox.layout().addMultiCellWidget(VerticalSpacer(self.paramsBox), 7, 7, 0, 5)
    ...
    def setDefaultValues(self,prefix=None,prop_code=None,prop_number=None):
        ...
        if prop_code is not None and prop_code!="":
            ...
            try:
                ...
                self.setDirectory(os.path.join('/data','visitor',directory_prefix,prop,curr_date_str))
            else:
                inhouse="%s%s" % (prop_code,prop_number)
                self.setDirectory(os.path.join('/data',directory_prefix,'inhouse',inhouse,curr_date_str))
        else:
    ...
    def buildDirectoryTemplate(self,protein_acronym,sample_name):
        ...
        if self.proposalCode is not None:
            ...
            if protein_acronym is None:
                if dir_prop == "inhouse":
                    dir_str=os.path.join('/data',dir_prefix,dir_prop,prop_str,curr_date_str)
                else:
                    dir_str=os.path.join('/data',dir_prop,dir_prefix,prop_str,curr_date_str)                    
            else:
                curr_sample=os.path.join(protein_acronym,sample_name)
                if dir_prop == "inhouse":
                    dir_str=os.path.join('/data',dir_prefix,dir_prop,prop_str,curr_date_str,curr_sample)
                else:
                    dir_str=os.path.join('/data',dir_prop,dir_prefix,prop_str,curr_date_str,curr_sample)
        else:
    ...
    def setParamState(self,param_id,state):
        self.paramDict[param_id].setPaletteBackgroundColor(DataCollectParametersWidget.PARAMETER_STATE[state])
