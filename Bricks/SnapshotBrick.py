...
class SnapshotBrick(BlissWidget):
    ...
    def __init__(self, *args):
        ...
        QObject.connect(self.settingsButton, SIGNAL('clicked()'), self.openSettingsDialog)
    ...
    def takeSnapshot(self):
        if not os.path.isdir(self.directory):
            try:
                ...
                logging.getLogger().info("SnapshotBrick: created the directory %s" % self.directory)
    ...
    def openSettingsDialog(self):
        if self.isRunning():
            if self.settingsDialog is None:
                ...
                try:
                    self.settingsDialog.setIcons(icons_list[2],icons_list[3],icons_list[4],icons_list[5])
                except IndexError:
                    pass

            self.settingsDialog.setParameters(self.prefix,self.directory,self.fileIndex,self.formatType)
...
class SettingsDialog(QDialog):
    ...
    def browseButtonClicked(self):
        ...
        d=get_dir.getExistingDirectory(self.fileDirectory.text(),self,"",\
