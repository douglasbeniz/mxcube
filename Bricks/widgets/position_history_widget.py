...
CollectionMethod = namedtuple('CollectionMethod', 
                              ['DISCRETE', 'HELICAL', 'MESH', 'MULTI_POS'])

COLLECTION_METHOD = CollectionMethod(0, 1, 2, 3)

CollectionMethodName = \
    namedtuple('CollectionMethodName',  ['DISCRETE', 'HELICAL', 
                                         'MESH', 'MULTI_POS'])

COLLECTION_METHOD_NAME = \
    CollectionMethodName('Discrete', 'Helical', 'Mesh', 
                         'Multi position')
        
ExperimentType = namedtuple('ExperimentType', 
                            ['SAD', 'SAD_INV', 'OSC', 'MAD',
                             'BURN', 'MAD_INV', 'SCREENING'])
...
class PositionHistoryBrickWidget(QWidget):
    def __init__(self, parent = None, name = "position_history",
        ...
        self.delete_point_button = \
            QPushButton("Remove", self, "delete_point_button")
    ...
    def selected_points(self):
        ...
        for index in self.__selected_points_idx():
            points.append((self.__stored_positions[index],
                           self.point_list_box.item(index).text(), index))
