...
class TemperaturePanel(qt.QWidget):
  ...
  def setTemperatures(self, temp_list):
    try:
    	self.temp_list = list(map(float, temp_list[:self.nccds]))
    except:
  ...
  def paintEvent(self, event):
    ...
    for i in range(n):
      ...
      for j in range(n):
        ...
        brush = qt.QBrush(c)
        p.fillRect(x+1, j*step_y+1, x+step_x-1, (j+1)*step_y-1, brush) 
...
class AdscTemperatureMonitorBrick(BaseComponents.BlissWidget):
  def __init__(self, *args, **kwargs):
    ...
    self.lblStatus.setSizePolicy(qt.QSizePolicy.MinimumExpanding, qt.QSizePolicy.Fixed)
  ...
  def propertyChanged(self, property, old_value, new_value):
    if property == 'mnemonic':
      if self.detector_ho is not None:
        self.disconnect(self.detector_ho, "valueChanged", self.temperaturesUpdated)
